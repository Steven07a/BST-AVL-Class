#ifndef AVL_FUNCTIONS_H_
#define AVL_FUNCTIONS_H_
...
void tree_print_debug(tree_node<T>* root, int level = 0,
    ...
    if (root) {
        ...
        outs << setw(4 * level) << "" << "[" << root->_item << "] H:" << root->height() << " BF: " << root->balance_factor() << endl;
...
bool tree_erase(tree_node<T>*& root, const T& target) {
    ...
    if (target > root->_item) {
        bool eraseBool = tree_erase(root->_right, target);
        if (abs(root->balance_factor()) >= 2) {
            rotate(root);
        }
        root->update_height();
    ...
    if (target < root->_item) {
        bool eraseBool = tree_erase(root->_left, target);
        if (abs(root->balance_factor()) >= 2) {
            rotate(root);
        }
        root->update_height();
